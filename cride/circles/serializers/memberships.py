...
class AddMemberSerializer(serializers.Serializer):
    ...
    user = serializers.HiddenField(default=serializers.CurrentUserDefault()) #gracias a que le mandamos el context , request  en el view

    def validate_user(self, data):
        """Verify user isn't alredy a member"""
        circle = self.context['circle']
        user = data
        q = Membership.objects.filter(circle=circle, user=user)
        if q.exists():
            raise serializers.ValidationError('User is alredy member of this circle')
        return data

    def validate_invitation_code(self, data):
        """Verify that code exist and that it is related to the circle"""
    ...
    def create(self, data):
        """Create new circle member and update"""
        circle = self.context['circle']
