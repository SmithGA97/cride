...
class RideModelSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, data):
        """Allow updates only befores departure date."""
        now = timezone.now()
...
class CreateRideSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        # Vamos a escribir cuales campos de Ride no necesitamos
    ...
    def  validate_departure_date(self, data):
        ...
        if data < min_date:
            raise serializers.ValidationError(
                'Departure time must be at least pass the next 20 min window'
            )
        return data
    ...
    def validate(self, data):
        ...
        Verify that the person who offer the ride is member and
        also the same user making the request
        """
        ...
        if data['arrival_date'] <= data['departure_date']:
            raise serializers.ValidationError('Departure date must be happen after arrival date')
...
class JoinRideSerializer(serializers.ModelSerializer):
    ...
    def validate_passenger(self, data):
        """Verify if passenger exists and is a circle member. """
        try:
    ...
    def validate(self, data):
        ...
        if ride.passengers.filter(pk=self.context['user'].pk).exists():
            raise serializers.ValidationError('Passeger is alredy in this trip')
